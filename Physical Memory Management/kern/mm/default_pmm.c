...
default_init_memmap(struct Page *base, size_t n) {
    ...
    base->property = n;//连续内存空闲块的大小为n，属于物理页管理链表，头一个空闲页块要设置数量
    SetPageProperty(base);
    nr_free += n;//说明连续有n个空闲块，属于空闲链表
    list_add_before(&free_list, &(base->page_link));//插入空闲页链表中，初始化每个空闲页后，将其插入到链表，每次都插入到节点前面，方便按地址排序
}

static struct Page *
default_alloc_pages(size_t n) {
    assert(n > 0);
    if (n > nr_free) { //如果所有的空页的加起来的大小都不够，那直接返回NULL
        return NULL;
    }
    struct Page *page = NULL;
    list_entry_t *le = &free_list;//从空闲块链表的头指针开始
    // TODO: optimize (next-fit)
    // 查找 n个或以上空闲页块,若找到则判断是否大过n,则将其拆分并将拆分后的剩下的空闲页块加回到链表中
    while ((le = list_next(le)) != &free_list) {//依次往下寻找直到回到头指针处，即已经遍历一次
        //此处 le2page 就是将 le的地址 -page_link 在Page的偏移,从而找到 Page 的地址
